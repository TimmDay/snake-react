...
class Snake extends React.Component {
   constructor(props) {
       ...
       this.state = {
         px: 10,
         py: 10, // start snake head position x and y
         appleX: 15,
         appleY: 15, // start apple position x and y
         gridSize: 25,
         tileCount: 25, // grid size, tile count. should multiply to canvas side dimension (height).
         xVelocity: 0,
         yVelocity: 0, //x and y velocity (starting speed in x,y directions). works off key press so 0 to start
         trail: [{x:10, y:10}], //to store positions of the trailing snake body pixels
         tail: 5, // tail length
       }
   }
  ...
  componentWillUnmount() {
   ...
   game = () => {
    ...
    this.state.trail.forEach((trailBit) => {
      ...
      ctx.fillRect(trailBit.x * this.state.gridSize, trailBit.y * this.state.gridSize, this.state.gridSize-2, this.state.gridSize-2); 
      
      //if snake hits tail, reset to short tail
      if (trailBit.x === this.state.px && trailBit.y === this.state.py) this.setState({tail: 5}); 
    })
    ...
    while(this.state.trail.length > this.state.tail) this.state.trail.shift(); //remove leading elements of trail arr until it has length tail

    // EAT APPLE! when apple position === snake head position
    if(this.state.appleX === this.state.px && this.state.appleY === this.state.py) {
    ...
    // paint apple
    ctx.fillStyle="red";
    ctx.fillRect(this.state.appleX * this.state.gridSize, this.state.appleY * this.state.gridSize, this.state.gridSize - 2, this.state.gridSize - 2); 
}
  ...
  keyPush = (evt) => {
   ...
   render () {
    return (
      <canvas id="snake-game-canvas" width="635" height="625"></canvas>
    )
   }
}
